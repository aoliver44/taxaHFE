...
read_in_microbiome <- function(input, meta = metadata, cores = opt$ncores) {
  ...
  if (strsplit(basename(input), split="\\.")[[1]][2] %in% c("tsv","txt")) {
    hData <- suppressMessages(vroom::vroom(file = input, delim = "\t", skip = 0, .name_repair = "minimal", num_threads = as.numeric(cores)) %>% dplyr::select(., -any_of(c("NCBI_tax_id", "clade_taxid"))))
  } else {
    hData <- suppressMessages(vroom::vroom(file = input, delim = ",", skip = 0, .name_repair = "minimal", num_threads = as.numeric(cores)) %>% dplyr::select(., -any_of(c("NCBI_tax_id", "clade_taxid"))))
  }
...
calc_class_frequencies <- function(input = metadata, feature_type = opt$feature_type) {
  ...
  if (feature_type == "factor") {
    ...
    class_frequencies <- as.numeric(opt$subsample)
    assign("class_frequencies", class_frequencies, envir = .GlobalEnv)
...
taxaHFE_competition <- function(input = hData, feature_type = opt$feature_type, cores = opt$ncores, output) {
    ...
    for (parent_trial in children_counts[children_counts$count > 1, ][,1] %>% dplyr::pull()) {
      ...
      if (NCOL(hData_parent_merge_cor_subset) < 3) {
        ...
        if (feature_type == "factor") {
          ...
          model <- ranger::ranger(as.numeric(feature_of_interest) ~ . , data = hData_parent_merge_cor_subset, importance = "impurity_corrected", seed = 42, sample.fraction = class_frequencies, num.threads = as.numeric(cores))
          model_importance <- as.data.frame(model$variable.importance) %>% tibble::rownames_to_column(., var = "taxa")
          for (seed in sample(1:1000, nperm)) {
            model_tmp <- ranger::ranger(as.numeric(feature_of_interest) ~ . , data = hData_parent_merge_cor_subset, importance = "impurity_corrected", seed = seed, sample.fraction = class_frequencies, num.threads = as.numeric(cores))
            model_importance_tmp <- as.data.frame(model_tmp$variable.importance) %>% tibble::rownames_to_column(., var = "taxa")
            suppressWarnings(model_importance <- merge(model_importance, model_importance_tmp, by = "taxa"))
    ...
    for (parent_trial in children_counts[children_counts$count > 1, ][,1] %>% dplyr::pull()) {
      ...
      if (NCOL(hData_parent_merge_cor_subset) < 4) {
        ...
        if (feature_type == "factor") {
          ...
          ## this will help us decide if the PARENT (species) or the CHILD (sub-speceies)
          ## brings more information to the table with regards to feature_of_interest
          model <- ranger::ranger(as.factor(feature_of_interest) ~ . , data = hData_parent_merge_cor_subset, importance = "impurity_corrected", seed = 42, sample.fraction = class_frequencies, num.threads = as.numeric(cores))
          model_importance <- as.data.frame(model$variable.importance) %>% tibble::rownames_to_column(., var = "taxa")
