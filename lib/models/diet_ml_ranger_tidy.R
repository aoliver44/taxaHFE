...
folds <- rsample::vfold_cv(train, v = as.numeric(opts$folds), strata = feature_of_interest, repeats = 3)
...
training_cor <- mikropml:::group_correlated_features(train %>% dplyr::select(where(is.numeric)) %>% dplyr::select(., -dplyr::any_of(c("feature_of_interest", "subject_id"))), 
                                                      corr_thresh = as.numeric(opts$cor_level), group_neg_corr = T)
...
last_best_mod <- 
  ...
  parsnip::set_engine("ranger", num.threads = as.numeric(opts$ncores), importance = "none") %>% 
...
full_results$program <- opts$program
...
# ggplot2::ggsave(plot = hyperpar_perf_plot, filename = paste0(dirname(opts$OUTPUT), "/ml_analysis/", "training_performance.pdf"), width = 7, height = 2.5, units = "in")
# 
# hyperpar_tested_plot <- autoplot(search_res, type = "parameters") + 
#   labs(x = "Iterations", y = NULL)
# ggplot2::ggsave(plot = hyperpar_tested_plot, filename = paste0(dirname(opts$OUTPUT), "/ml_analysis/", "hyperpars_tested.pdf"), width = 7, height = 2.5, units = "in")
